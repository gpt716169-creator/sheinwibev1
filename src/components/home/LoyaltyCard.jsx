...
export default function LoyaltyCard({ points = 0, totalSpent = 0, onOpenDetails }) {
    ...
    return (
        <div className={`w-full h-52 perspective-1000 cursor-pointer group select-none relative z-10 ${shake ? 'animate-[shake_0.5s_ease-in-out]' : ''}`} onClick={handleCardClick}>
            <div className={`relative w-full h-full duration-700 preserve-3d transition-transform ${isFlipped ? 'rotate-y-180' : ''}`}>
                ...
                <div className={`absolute inset-0 backface-hidden rounded-2xl p-6 bg-gradient-to-br ${info.color} shadow-[0_0_30px_rgba(255,255,255,0.1)] overflow-hidden flex flex-col justify-between z-20`}>
                    ...
                    {/* Снег на карточке (исчезает при первой тряске - можно сделать состояние, но пусть будет всегда для красоты) */}
                    <div className="absolute top-2 right-4 text-white/40 text-xl animate-pulse">❄️</div>
